import { ArrowRight, Calendar, GlassWater, ShieldCheck, Users } from "lucide-react";
...
const rooms = [
	{
		...
		description: "An intimate space overlooking our main wood-fired hearth. Perfect for celebrations that want to feel the energy of the kitchen while maintaining privacy.",
	...
	},
	{
		...
		description: "Surrounded by our curated collection of vintage wines. A quiet, sophisticated sanctuary for business dinners or family milestones.",
	...
	},
	{
		...
		description: "Our largest space, featuring cathedral ceilings and a dedicated copper-clad bar. Ideal for corporate events and wedding receptions.",
...
const PrivateDining: React.FC = () => {
	...
	const labelClass = "mb-1.5 block text-xs font-medium uppercase tracking-[0.12em] text-white/40";
			...
			<div className="mx-auto max-w-7xl px-6 py-7 md:px-10">
				...
				<section className="mt-16 grid grid-cols-1 gap-8 md:grid-cols-3">
					<div className="flex flex-col items-center text-center">
						...
						<p className="mt-2 text-sm text-[#9a8a7a]">Rooms for 8 to 50 guests.</p>
					</div>
					<div className="flex flex-col items-center text-center">
						<div className="mb-4 text-[#e8a87c]">
							<GlassWater size={28} />
						...
						<p className="mt-2 text-sm text-[#9a8a7a]">Chef-designed tasting or family-style.</p>
					</div>
					<div className="flex flex-col items-center text-center">
						<div className="mb-4 text-[#e8a87c]">
							<ShieldCheck size={28} />
						...
						<p className="mt-2 text-sm text-[#9a8a7a]">Personal coordinator for every event.</p>
					</div>
				</section>
				...
				<section className="mt-24 space-y-24">
					{rooms.map((room, i) => (
						<div key={room.name} className={`grid grid-cols-1 items-center gap-12 lg:grid-cols-2 ${i % 2 === 1 ? 'lg:rtl' : ''}`}>
							...
							<div className={i % 2 === 1 ? 'lg:ltr' : ''}>
								...
								<ul className="mt-8 space-y-4">
									{['Audiovisual support available', 'Private copper bar access', 'Custom floral arrangements', 'Sommelier-led pairings'].map((item) => (
				...
				{/* Enquiry Form */}
				<section id="enquire" className="mt-32 rounded-[3.5rem] bg-[#1a140d] border border-white/5 overflow-hidden">
					<div className="grid grid-cols-1 lg:grid-cols-2">
						<div className="p-10 md:p-16 lg:border-r lg:border-white/5">
							<p className="text-xs font-bold uppercase tracking-[0.25em] text-[#e8632d]">
								Plan Your Event
							</p>
							<h2 className="mt-4 font-serif text-4xl md:text-5xl">Enquire Now</h2>
							...
							{submitted ? (
								<div className="mt-12 text-center py-10">
									...
									<p className="mt-4 text-[#9a8a7a]">We'll be in touch very soon to discuss your special event.</p>
									<button 
								...
								<form onSubmit={handleSubmit} className="mt-10 space-y-6">
									<div className="grid grid-cols-1 gap-6 md:grid-cols-2">
										<div>
											...
											<input id="name" type="text" required placeholder="Alexander Knight" className={inputClass} />
										</div>
										<div>
											<label htmlFor="email" className={labelClass}>Email Address</label>
											<input id="email" type="email" required placeholder="alex@company.com" className={inputClass} />
										</div>
									</div>
									<div className="grid grid-cols-1 gap-6 md:grid-cols-2">
										<div>
											<label htmlFor="date" className={labelClass}>Ideal Date</label>
											<input id="date" type="date" required className={inputClass} />
										</div>
										<div>
											<label htmlFor="guests" className={labelClass}>Expected Guests</label>
											<input id="guests" type="number" placeholder="20" className={inputClass} />
										</div>
									</div>
									<div>
										<label htmlFor="type" className={labelClass}>Event Type</label>
										<select id="type" className={inputClass}>
											<option value="" className="bg-[#1a140d]">Select type...</option>
											<option value="corporate" className="bg-[#1a140d]">Corporate Dinner</option>
											<option value="wedding" className="bg-[#1a140d]">Wedding Reception</option>
											<option value="birthday" className="bg-[#1a140d]">Birthday Celebration</option>
											<option value="other" className="bg-[#1a140d]">Other Milestone</option>
										</select>
									</div>
									<div>
										<label htmlFor="notes" className={labelClass}>Additional Details</label>
										<textarea id="notes" rows={4} placeholder="Tell us about your vision, dietary needs, or specific room preferences..." className={`${inputClass} resize-none`} />
									</div>
									<button
										type="submit"
									...
									>
										Send Enquiry <ArrowRight size={18} className="transition-transform group-hover:translate-x-1" />
									</button>
								</form>
							)}
						</div>
